...
from telemetrydata.sources.dev_serial import SerialNode
from telemetrydata.schemas.shtrack import gpstime_schema,text_schema,race_solar_mi_2_schema,vaisala_schema
...
##########################################
    ...
    def enable_telemetry(self,enable=True):
        ...
        if (enable and (self.telemetry_enable == False)):
            ...
            self.source_manager.add_schema(gpstime_schema())
            self.source_manager.add_schema(text_schema())
            self.source_manager.add_schema(race_solar_mi_2_schema())
            self.source_manager.add_schema(vaisala_schema())
