...
int mpu_reset_fifo(void)
{
    ...
    if (chip_cfg.dmp_on) {
        ...
        if (i2c_write(MPU9250_DEFAULT_ADDRESS, MPU9250_INT_ENABLE, 1, &data))
            return -1;
        if (i2c_write(MPU9250_DEFAULT_ADDRESS, MPU9250_FIFO_EN, 1, &chip_cfg.fifo_enable))
            return -1;
    }
    return 0;
}
...
#ifdef AK89xx_SECONDARY
        mpu_set_compass_sample_rate(min(chip_cfg.compass_sample_rate, MAX_COMPASS_SAMPLE_RATE));
#endif
...
static int setup_compass(void)
{
    ...
    //set scale mag:
    chip_cfg.mag_offset[0] = 0;
    chip_cfg.mag_offset[1] = 0;
    chip_cfg.mag_offset[2] = 0;
#ifdef MPU_DEBUG
    Serial.print("Compass sens: "); Serial.print(chip_cfg.mag_sens_adj[0]); Serial.print(" ");
    Serial.print(chip_cfg.mag_sens_adj[1]); Serial.print(" ");
    Serial.print(chip_cfg.mag_sens_adj[2]); Serial.println();
#endif
...
int8_t mpu_get_compass_raw(int16_t *magX, int16_t *magY, int16_t *magZ)
{
...
#ifdef AK89xx_BYPASS
    if (i2c_read(chip_cfg.compass_addr, AKM_REG_ST1, 8, tmp))
        return -2;
    tmp[8] = AKM_SINGLE_MEASUREMENT;
    if (i2c_write(chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp+8))
        return -3;
#else
    ...
    *magX = ((long)*magX * chip_cfg.mag_sens_adj[0]) >> 8;
    *magY = ((long)*magY * chip_cfg.mag_sens_adj[1]) >> 8;
    *magZ = ((long)*magZ * chip_cfg.mag_sens_adj[2]) >> 8;
...
/**
 *  @brief      Compass Calibrate for heading
 *  @param[in]  seconds seconds to calibrate
 *  @return     0 if successful.
 */

int8_t mpu_calibrate_mag(uint8_t seconds){
    uint8_t count;
    int16_t mx,my,mz;
    int16_t mmmag_x[2]; //max, min
    int16_t mmmag_y[2];
    int16_t mmmag_z[2];
    
    seconds *= 4; //lo multiplico por 4 medidas por segundo 
    
    for (count = 0; count >seconds; count++) {
        mpu_get_compass_raw(&mx, &my, &mz);
        if (mx>mmmag_x[0]){
            mmmag_x[0] = mx; //el maximo
        }
        if (mx<mmmag_x[1]){
            mmmag_x[1] = mx; //el minimo
        }
        if (my>mmmag_y[0]){
            mmmag_y[0] = my; //el maximo
        }
        if (my<mmmag_y[1]){
            mmmag_y[1] = my; //el minimo
        }
        if (mz>mmmag_z[0]){
            mmmag_z[0] = mz; //el maximo
        }
        if (mz<mmmag_z[1]){
            mmmag_z[1] = mz; //el minimo
        }
        
        delay(250);
    }
    //calculo del punto medio offset: max+min/2
    chip_cfg.mag_offset[0] = (mmmag_x[0]+mmmag_x[1])/2;
    chip_cfg.mag_offset[1] = (mmmag_y[0]+mmmag_y[1])/2;
    chip_cfg.mag_offset[2] = (mmmag_z[0]+mmmag_z[1])/2;
    
}

/**
 *  @brief      Compass degrees heading
 *  @return     deg units 
 */

int8_t mpu_get_compass_heading(void){
    int16_t mx,my,mz;
    float deg;
    mpu_get_compass_raw(&mx, &my, &mz);
    
    //le quito el offset
    mx -= chip_cfg.mag_offset[0];
    my -= chip_cfg.mag_offset[1];
    mz -= chip_cfg.mag_offset[2];
    
    deg = atan2((float)my, (float)mx);
